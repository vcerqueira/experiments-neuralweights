"""Open HPO search comparing WASP (regressor-based) vs standard approaches.

This script uses a conformal regression meta-model that predicts P(MASE > threshold)
instead of a binary classifier predicting P(exceeds baseline).
"""
import warnings
...
from src.search import train_meta_regressor
from src.config_callbacks import (
    CONFIG_SAMPLERS,
    AutoConfigWithRegressorCallback,
...
if __name__ == "__main__":
    ...
    for i, target_dataset in enumerate(all_datasets):
        ...
        meta_regressor, reg_feature_columns = train_meta_regressor(meta_train)
        ...
        # Variants with WASP callback (regressor-based)
        config_wasp = {
            alias: AutoConfigWithRegressorCallback(
